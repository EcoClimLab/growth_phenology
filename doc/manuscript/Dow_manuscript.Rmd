...
At SCBI, patterns were quite similar to those described above for $T_{max}$, with similar timing and strengths of correlations across the two temperature variables (Figs. ` r climwin_rp_DOY25_SCBI` - `climwin_dp_DOY25_SCBI`).
...
Similarly, annual growth recorded in tree-ring records, $RW$, rarely displayed a significant correlation to spring  $T_{max}$ (Fig. `r tree_rings` or  $T_{min}$ (Fig. `r tree_rings_TMIN`).
