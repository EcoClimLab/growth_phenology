...
\newpage
*(Do we want a table of species here?? Columns would be Species, xylem porosity, n trees, n tree-years (after cleaning). Details will be in an SI table)*

```{r species_table, eval=TRUE, echo=FALSE, warning=FALSE}
library(knitr)
species_table <- read.csv("tables_figures/species_table.csv",stringsAsFactors = FALSE, check.names = FALSE)
knitr::kable(species_table) 
```
...
(*The following seems out of date, right? Scripted cleaning is not described.* -- *Manually cleaning was done for pre-analysis data. Added some text to clarify.*)
The dendroband data were manually cleaned by visual inspection before being used for modeling. Three classes of mistakes: 1. Error in measurement - weekly measurement was drastically different from previous week and following week. Mistakes were removed from the raw data before modeling. 2. Band slip or stuck - measurements freeze and remain unchanged until sudden jump followed by normal growth pattern. Band slips were sometimes followed by measurements indicating the band became stuck; in these cases, the tree was thrown out for the corresponding year. In cases where the slip was followed by normal growth, the initial slip-point was removed, and the following points were shifted down to the pre-slip level. 3. Other - cases where data was clearly wrong but with unknown causes. In cases where several measurements were an issue, they were simply removed. If there were no clear solutions to ‘fix’ the data, the entire year was removed from the analysis.

We fit the model of @mcmahon_general_2015 to define *phenological dates and growth rates* (Fig. `r schematic`). This 5 parameter logistic model was fitted to each tag year's dendroband measurements in R: L + ((K - L) / (1 + 1/theta * exp(-(r * (doy - doy_ip) / theta)) ^ theta))
Where L and K are lower and upper asymptotes of the model, theta is a tuning parameter controlling the slope of the curve toward the upper asymptote, r is the slope of the curve at the inflection point, doy_ip is the day of year where the inflection point occurs, and doy are the modeled days of year, in this case, 1-365. *We note that the maximum growth rate does not always occur on the doy_ip, and is controlled magically by theta somehow.* Using the optimized parameters for each tree year, we could then determine the DBH on every day within that year, giving us the ability to extract the intra-annual growth variables we were interested in. 
...
# Created in RScripts/additional_figures.R
knitr::include_graphics("tables_figures/tree_rings.tif")
```
...
Thus, while our analysis is limited to two sites, we expect that the finding that spring temperatures have limited effect on annual tree grwoth will hold generall.y.
...
Yet, an earlier start to the growing season does not imply a longer growing season [Table `r hypotheses`, Figs `r pheno_Tsensitivity`, `r DOYtiming`;  @zani_increased_2020].
