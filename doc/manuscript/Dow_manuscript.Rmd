...
growth_pheno_by_xylem = 2  #if we include more sites, we'll make this multi-panel
...
climwin_caption <- "Figure 3. Landscapes of relationships between the day of year on which 25% of annual growth is achieved (DOY_25) and temperature in prior weeks for ring- and diffuse-porous trees. Shown is climwin output indicating the change in AIC relative to a null model (first panel) and linear coefficients (second panel). Window Open and Window Close indicate number of weeks prior to DOY_25 (ring-porous: DATE; diffuse-porous: DATE). Black boxes indicate the optimal window (i.e., that with lowest dAIC)."
...
model_summary_caption <- "Figure 5. (PLACEHOLDER FOR FIGURE WE DISCUSSED AT LAB MEETING)"

climwin_rp_DOY25_SCBI_short_caption <- "Figure S1. Full climwin output for DOY 25 for ring porous species at SCBI."
climwin_dp_DOY25_SCBI_short_caption <- "Figure S1. Full climwin output for DOY 25 for diffuse porous species at SCBI."
DOYtiming_caption <- "Figure 6. Day of year where growth milestones were achied at the Smithonian Conservation Biology Institute."
```
...
#### (5. Here, we...)
Using many measurements taken within a year, we can model the growth of individual trees and extract data on growth milestones and traits for interspecific, intraspecific, and temporal comparison (McMahon & Parker, 2014). This allows the determination of variable responses of key tree species within a forest, meaning a better understanding of future growth under projected climate conditions. Here, we use biweekly dendrometer band measurements (5 years @ Harvard Forest, 10 @ SCBI) for dominant tree species at each site (15 @ Harvard Forest, 4 @ SCBI)  to characterize intraannual stem-growth phenology within two forest dynamics plots in the Eastern USA, calculating for each tree in each year where data was available: the day of year (DOY) where 25, 50, and 75% annual growth was achieved; the maximum growth rate; the DOY where maximum growth rate was achieved; the total growth; and season length (75%-25% DOY) (Fig. `r schematic`). Correlation between these variables and Spring temperatures over the 15 year study period (1998-2003; 2011-2019) were assessed using a linear mixed model in a heriarchal bayesian framework.

We test the hypotheses that (1) stem-growth milestones are reached earlier in years with warmer early springs, but (2) maximum growth rates are independent of spring temperatures, and as a result (3) total annual growth increases in response to warmer spring temperatures (Table `r hypothesis_table`).
...
library(knitr)  
hypothesis_table <- read.csv(file = "tables_figures/hypothesis_table.csv")
knitr::kable(hypothesis_table, caption = "Table 1. Hypothesis table. (NEEDS FORMATTING...obviously!)")
```
...
```{r climwin, out.width="100%", echo=FALSE, fig.align="center", fig.cap= cum_growth_caption}
# Created in RScripts/additional_figures.R
