...
$\theta$ is a tuning parameter controlling the slope of the curve toward the upper asymptote, $r$ is the slope of the curve at the inflection point, $DOY_{ip}$ is the day of year where the inflection point in growth rate occurs, and $DOY$ are the modeled days of year (*i.e.*, julian days). 
*We note that the maximum growth rate does not always occur on the doy_ip, and is controlled magically by theta somehow. (???)* 
Using the optimized parameters for each tree-year, we could then determine the $DBH$ on every day within that year, giving us the ability to extract the intra-annual growth variables of interest (Fig. `r schematic`). 
