...
add_header_above(c(" ", " ", " ", " ", "dendrometer bands" = 2, "tree cores"=2 )) %>%
  pack_rows("", 1, 10, latex_gap_space = "1em", colnum = 4, hline_before = FALSE) %>%
  pack_rows("", 11, 12, latex_gap_space = "1em", colnum = 4, hline_before = FALSE) %>%
  pack_rows("", 13, 15, latex_gap_space = "1em", colnum = 4, hline_before = FALSE) %>%
...
library(knitr)
library(kableExtra)
mean_growth_parameters <- read.csv("tables_figures/phenology_parameters.csv",  col.names = c("", "ring","diffuse","ring", "diffuse"), stringsAsFactors = FALSE, check.names = FALSE)
...
add_header_above(c(" ",  "SCBI" = 2, "Harvard Forest"=2 )) %>%
    ...
    pack_rows("Phenological sensitivity to mean max. T in critical T window (days/°C)", 13, 22, latex_gap_space = "1em", colnum = 5, hline_before = FALSE) %>%
    add_indent(14:17)%>%
    add_indent(19:22)%>%
kable_styling(latex_options = c("scale_down", "hold_position"), protect_latex = T) 
```
...
add_header_above(c(" " = 2,  "n. correlations to monthly max. T significant at p=0.05" = 14)) %>%
