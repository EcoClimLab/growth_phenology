...
Similarly, with the exception of ring-porous $DOY_{75}$ at SCBI and ring-porous $DOY_{g_{max}}$ at HF, the stem growth milestones $DOY_{25}$, $DOY_{50}$, $DOY_{75}$,  and $DOY_{g_{max}}$ decreased with mean $T_{max}$ during the critical temperature window (Figs. `r schematic_summary`, `r DOY_timing`c-d; `r EDF_pheno_all`).
...
Specifically, $g_{max}$, which occurred very close to $DOY_{50}$ (on $DOY_{g.max}$; `r EDT_phenology_parameters`), displayed extremely small negative changes (HF) or changes in opposite directions (SCBI) in relationship to mean $T_{max}$ during the critical temperature window for ring- and diffuse- porous species.
