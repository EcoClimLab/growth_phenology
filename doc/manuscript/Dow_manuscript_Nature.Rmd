...
Whereas the length of time between canopy greenup and senescence (*i.e.,* the day when greenness dropped below 90% of its peak) increased with CTW $T_{max}$ (Fig. `r DOY_timing`a-b)<!--specify which species group used?-->, there was no consistent lengthening of $L_{PGS}$ (Fig. `r schematic_summary`, `r EDF_pheno_all`).<!--Note to Cam: double check this-->
...
Specifically, $g_{max}$, which occurred very close to $DOY_{50}$, displayed extremely small negative changes (HF) or changes in opposite directions (SCBI) in relationship to CTW $T_{max}$ for ring- and diffuse- porous species.
$\Delta DBH$ displayed a weak positive relationship (+0.029-0.08 mm/ $^\circ$C) with CTW $T_{max}$ at SCBI and a weak negative relationship (-0.2-0.03mm/ $^\circ$C) at HF (`r EDF_pheno_Tmax_climwin`). 
