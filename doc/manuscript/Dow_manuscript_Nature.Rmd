...
<!--LENGTH: The typical length of an article with 3-4 modest display items (figures and tables) is 2000-2500 words (summary paragraph plus body text). We're currently at 3005 words and 1 modest + 2 large display items. Articles typically have 30-50 references (currently at 59)-->
...
Seasonal milestones for both canopy foliage phenology and stem growth occurred 3-16 days earlier at SCBI than at Harvard Forest (Fig. `r DOY_timing`, `r  EDT_phenology_parameters`).<!--cool to note, but is it necessary? Especially considering how broad "seasonal milestones for BOTH..." is. KAT: I'd like to try to keep it in because it links to I think we should keep it in because it links-->
...
Similarly, with the exception of ring porous $DOY_{75}$ at SCBI, the stem growth milestones $DOY_{25}$, $DOY_{50}$, and $DOY_{75}$ decreased with mean $T_{max}$ during the CTW (Figs. `r schematic_summary`, `r DOY_timing`c-d; `r EDF_pheno_all`).
...
Specifically, $g_{max}$, which occurred very close to $DOY_{50}$, displayed extremely small negative changes (HF) or changes in opposite directions (SCBI) in relationship to mean $T_{max}$ during the CTW for ring- and diffuse- porous species.
$\Delta DBH$ displayed a weak positive relationship (+0.029-0.08 mm/ $^\circ$C) with mean $T_{max}$ during the CTW at SCBI and a weak negative relationship (-0.2-0.03mm/ $^\circ$C) at HF (`r EDF_pheno_Tmax_climwin`). 
...
Our observations suggest that the deceleration of stem expansion (starting around $DOY_{50}$), which occurs mid-summer near the time of peak canopy greenness (`r EDF_growth_curves_all`)[@finzi_carbon_2020;@etzold_number_2022], is likely driven by cues other than photosynthate limitation, such as water stress, daylength, or sink limitation[@zweifel_why_2021;@etzold_number_2022;@zani_increased_2020;@keenan_timing_2015;<!--@fu_variation_2014-->;@delpierre_wood_2016;@delpierre_temperate_2016].
Our tree-ring analysis (Fig. `r tree_rings`) demonstrates that the primary effect of warming temperatures on annual ring-width is not an augmentation through an earlier start to growth, but rather a reduction associated to high $T_{max}$ during the peak growing season[@dorangeville_drought_2018;@etzold_number_2022].

<!--expectation of extra C, not going to ANPP_woody (sink limitation)-->
Our finding that interannual variation in stem growth is more strongly linked to conditions during the peak growing season than to the length of the season of active stem growth aligns with recent studies on annual stem growth[@etzold_number_2022;@tumajer_limitation_2022;@delpierre_wood_2016] and with parallel findings for NEE[@zhou_explaining_2016;@fu_maximum_2019]. 
However, there is also a disconnect with findings that NEE is at least modestly greater in years with warm springs[@keenan_net_2014] or long growing seasons[@zhou_explaining_2016;@fu_maximum_2019;@finzi_carbon_2020].
...
We show that the extra C fixation in years with warm springs does not substantially augment stem diameter growth in the current or following year (`r EDF_tree_rings_TMAX_extended`).

Although stem diameter increments are routinely used to calculate $ANPP_{woody}$[@refs], stem volume growth does not directly translate into C content, but rather can be partially decoupled through differences in wood density or C content[@delpierre_wood_2016; @and_refs_therein].  
Thus, it is theoretically possible that extra C is allocated to enhanced cell wall thickening, a process that lags behind stem expansion[@cuny_woody_2015], or to a higher ratio of high-density latewood to lower-density earlywood, resulting in formation of more C-dense wood in years with warm springs.
...
*Rather, the most likely explanation of the observed lack of correlation between spring temperatures and stem growth is that $ANPP_{woody}$ is limited by sink factors, such that the expected increase photosynthate availability[@refs] (observed at Harvard Forest[@finzi_carbon_2020_but_verify]) does not enhance growth.*
*Indeed, such a phenomenon has been observed when GPP has been increased through lengthening of the growing season[@refs] or elevated CO~2~[@refs].*

<!--on the fate of extra C-->
Our results add to a growing body of evidence for a sink limitation of stem growth and $ANPP_{woody}$[@refs], wherein global change factors such as warming spring temperatures[@refs] or elevated CO~2~[@refs] enhance GPP, but woody growth is less responsive, if at all[@refs].
...
Indeed, in a situation where GPP was enhanced (through experimental elevation of CO~2~) but $ANPP_{woody}$ was constrained *(presumably by sink factors)*, additional C was released back to the atmosphere on relatively short time scales through enhanced respiration[@jiang_fate_2020].
Consistent with this, it has been observed that C loss through fall or winter respiration can offset gains from an earlier spring[@oishi_warmer_2018;@buermann_widespread_2018].
It is also possible that a buildup of NSCs that are not required for growth or other functions would feed back to reduce the rate of photosynthesis[@refs], such that sink limitation of $ANPP_{woody}$ reduces GPP relative to its potential in years with warm springs.
Either way, our results demonstrate that warm springs do not increase $ANPP_{woody}$ on an interannual time scale, contrasting with their tendency to augment _NEE_[@zhou_explaining_2016;@fu_maximum_2019;@keenan_net_2014].
...
Regardless of the influence of spring temperatures on C cycling within the ecosystem, our results make clear that the dominant effect of temperatures on tree growth occur not in the spring, but during the peak growing season. 
...
This finding is consistent with numerous tree-ring studies demonstrating strong sensitivity of growth to drought stress[@refs] or high temperatures[@anderson-teixeira_joint_2021] (which cause drought stress[@refs]) during the peak growing season.
