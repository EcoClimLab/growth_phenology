...
We find statistically, but not biologically, significant relationships between total growth and spring temperature at both sites in our dendrometer band analysis. 
However, the slope of the relationships are in opposite directions, with warming causing decreasing stem growth at Harvard Forest but increasing stem growth at SCBI. 
We believe this difference is due to severe drought conditions in 1999 which obviously caused abnormal growth patterns in that year (pheno_tsens). 
...
At SCBI, we see a very small increase in total stem growth (on average +0.05mm/$^\circ$C), suggesting that even in the absence of drought or incliment climate conditions during the growing season, stem growth is not enhanced in a meaningful way when spring temperatures are warmer, and peak stem growth is initiated earlier.  
At Harvard Forest, we see a more meaningful decrease in total growth in relation to warmer temperatures in the CTW (-0.15mm/$^\circ$C on average). We speculate that this result is likely a product of reductions in total stem growth related to drought damage in 1999. 
Due to the method of selection for CTW, years with significant departures from the norm can influence the analysis to a greater extent. Stem growth in 1999 was clearly impacted by climate conditions, and so the CTW selected by climwin maximizes the effect of these impacts (most likely the effects of drought) on stem growth. To examine this further, we reran our dendrometer band analysis using the month of May as the CTW. When we change the CTW to the month of May, we see results much closer to those found at SCBI (EDT...), with a similar advancement in growth timing but a much less pronounced effect of Tmax on total growth (-0.015mm/$^\circ$C on average, a tenfold decrease compared to the CTW optimized around the drought year). Interestingly, we once again see that there was no meaningful increase in total growth. 
The magnitude of the effect of the summer drought at Harvard Forest when compared to the relatively drought free period of SCBI illustrates the point that conditions during the peak growing season are much more influential to stem growth than conditions during the Spring. 
**We demonstrate that warmer springs either have little effect on, or even hasten, the deceleration of stem expansion and thereby have negligible effect on total annual stem growth for most species and locations (Fig. `r tree_rings`).** <!--repeated sentence/ CD made potential changes to the first one. See what you think. We should mention how 1999 was a drought year and also the warmest year in the CTW at HF for both wood types. This is potentially contributing to the negative correlations between CTW TMAX and total growth at HF. Maybe if there wasn't a severe drought in the warmest year we would see positive correlations like we do at SCBI. This is also probably worth mentioning when we discuss the influence of climate during peak growing season being greater than the spring climate. -->
...
(1) single day growth rates were $\ge 2.5$ standard deviations away from the mean for each xylem architecture (SCBI = 3, Harvard Forest = 11); 
...
(3) tree-years with small or negligible total growth ($\Delta DBH$ $\le 0.05$ mm; SCBI = 26, Harvard Forest = 253); 
(4) tree years where first intraannual measurement was later than the first spring survey (trees that were missed in the initial census; SCBI = 22, HF = 8).
...
This process was repeated using 2 and 3 standard deviations and the results of analyses were not qualitatively different. 
