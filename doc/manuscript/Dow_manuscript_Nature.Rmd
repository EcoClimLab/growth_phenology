...
As a result, the consequences of rising spring temperatures on stem growth -- or woody aboveground net primary productivity ($ANPP_{woody}$) on the ecosystem level -- may not be accurately represented in models[@delpierre_temperate_2016;@fatichi_moving_2014]. 
Understanding the sensitivity of $ANPP_{woody}$ to spring temperatures is central to predicting the future of the temperate deciduous forest C sink. 
...
In contrast, we have little evidence as to how stem growth and $ANPP_{woody}$ respond to warmer spring temperatures in deciduous forests[@savage_coordination_2021;@delpierre_temperate_2016].
...
Using dendrometer band measurements taken throughout multiple growing seasons at the Smithsonian Conservation Biology Institute (SCBI; Virginia, USA; n = `r n_dendro_trees_SCBI` trees from 2011-2020) and Harvard Forest (Massachusetts, USA; n = `r n_dendro_trees_HF` trees from 1998-2003), we fit a logistic growth model[@mcmahon_general_2015] to determine the days of year (DOY) when 25, 50, and 75% annual growth were achieved ($DOY_{25}$, $DOY_{50}$, $DOY_{75}$), peak growing season length ($L_{pgs}= DOY_{75}$-$DOY_{25}$), the maximum growth rate ($g_{max}$; occurs on average within 5 days of $DOY_{50}$), and total annual increment in diameter at breast height ($\Delta DBH$; Fig. `r schematic_summary`). 
...
CTW was identified by measuring the correlation between weekly $T_{max}$ and $DOY_{25}$ for all combinations of consecutive weeks from January 1 to mean $DOY_{25}$ for each xylem architecture-site combination (`r EDF_climwin`).
The CTW was defined as the weeks for which this correlation was strongest, and mean $T_{max}$ during this period (CTW $T_{max}$) used as our independent variable.   
...
For ring- and diffuse- porous species at both sites, warmer CTW $T_{max}$ resulted in earlier achievement of seasonal milestones.
...
Similarly, with the exception of ring porous $DOY_{75}$ at SCBI, the stem growth milestones $DOY_{25}$, $DOY_{50}$, and $DOY_{75}$ decreased with CTW $T_{max}$ (Figs. `r schematic_summary`, `r DOY_timing`c-d; `r EDF_pheno_all`).
...
Whereas the length of time between canopy greenup and senescence (*i.e.,* the day when greenness dropped below 90% of its peak) increased with the CTW $T_{max}$ of the porosity group containing the dominant canopy species at each site (diffuse porous at SCBI, ring porous at Harvard Forest; Fig. `r DOY_timing`a-b), there was no consistent lengthening of $L_{PGS}$ (Fig. `r schematic_summary`, `r EDF_pheno_all`).<!--Note to Cam: double check this-->

In contrast to the pronounced effects of CTW $T_{max}$ on the timing of growth, its effects on $g_{max}$ and $\Delta DBH$ were weak and inconsistent (Fig. `r schematic_summary`, `r EDF_pheno_all`). 
Specifically, $g_{max}$, which occurred very close to $DOY_{50}$, displayed extremely small negative changes (Harvard Forest) or changes in opposite directions (SCBI) in relationship to CTW $T_{max}$ for ring- and diffuse- porous species.
$\Delta DBH$ displayed a weak positive relationship (+0.029-0.08 mm/ $^\circ$C) with CTW $T_{max}$ at SCBI and a weak negative relationship (-0.2-0.03 mm/ $^\circ$C) at Harvard Forest (`r EDF_pheno_Tmax_climwin`). 
...
In total, our analysis included `r n_chronologies` chronologies representing `r n_cores_species` broadleaf species at `r n_cores_sites` sites distributed from Alabama (34.35$^\circ$N latitude) to Michigan (45.56$^\circ$N latitude) and spanning a 15 $^\circ$C range in April $T_{max}$.
...
There were even fewer significant positive correlations with March and May $T_{max}$: 0% and `r round(2/(142+66),2)*100`%, respectively (`r EDT_tree_ring_summary`).
...
$T_{min}$ generally exhibited weaker relationships to annual growth than $T_{max}$, with few significant correlations between spring $T_{min}$ and RWI (`r EDF_tree_rings_TMIN`).

To test whether warm springs might result in storage of non-structural carbohydrates (NSC) that would augment growth the following year[@richardson_seasonal_2013], we extended the analysis to examine correlations between RWI and $T_{max}$ in the previous year (`r EDF_tree_rings_TMAX_extended`).
This revealed little effect of previous spring temperatures on annual growth, with significant positive correlations of RWI to previous March or April $T_{max}$ for 5/142 ring porous chronologies and to previous April or May $T_{max}$ for 7/66 diffuse porous chronologies.
...
Results were qualitatively similar to the univariate correlations (Fig. `r tree_rings`), with significant (at p = 0.05) positive correlations to April $T_{max}$ for only 4% of chronologies and significant negative correlations with June-July $T_{max}$ for 77% of chronologies (`r EDT_tree_ring_summary`).
...
However, inconsistent with the concept that an earlier start to growth would increase $ANPP_{woody}$, we demonstrate that warmer springs either hasten the deceleration of stem expansion or otherwise fail to translate extended growing seasons into biologically significant increases in stem growth (Fig. `r schematic_summary`), and thereby have negligible effect on total annual growth for most species and locations (Fig. `r tree_rings`).
...
It remains theoretically possible that warm springs could augment $ANPP_{woody}$, which, although routinely calculated based on stem growth, can be partially decoupled from it through differences in wood density or C content[@delpierre_wood_2016].
...
Thus, it is unlikely that warm springs have a positive effect on total C content of annual rings or $ANPP_{woody}$. 
...
Indeed, when GPP of a mature forest was increased through experimental CO~2~ enrichment, $ANPP_{woody}$ remained unchanged, while additional C was released back to the atmosphere on relatively short time scales through enhanced respiration[@jiang_fate_2020].
...
It is possible that as spring warming continues, forests will adjust to directional changes in growing season length with an enhancement of $ANPP_{woody}$.
...
If extra photosynthate made available through a growing difference between GPP and $ANPP_{woody}$ is allocated to functions that relieve limitations on woody growth -- for example, by enhancing nutrient and water acquisition through enhanced allocation to roots[@zohner_how_2021;@liu_phenological_2022] -- it is possible that warming spring temperatures could ultimately increase $ANPP_{woody}$ through indirect mechanisms.
...
Indeed, the timing of peak growth in June and July (`r EDT_phenology_parameters`, `r EDF_growth_curves_all`) coincides with the timing of the greatest sensitivity of annual growth to $T_{max}$ (Fig. `r tree_rings`, `r EDT_tree_ring_summary`).
...
<!--*It is also worth noting that growth is often influenced by temperature of the preceding year(s) through its influence on production and storage of NSCs[@refs;@helcoski], and indeed we observe many significant effects of $T_{max}$ in the preceding growing season, albeit very few in the preceding spring (`r EDF_tree_rings_TMAX_extended`).*--><!--this sentence feels tangential, but discussion of these lag effects was requested by one reviewer.-->
...
Within the study period for the dendrometer band analysis (2011-2020), Spring temperatures (March + April $T_{max}$) and summer SPEI values (4-month value of August) were similar to the average prior to the study period (1970-2010). Prior to the study period, the average spring $T_{max}$ was 16.9 $degreeC$ and summer SPEI was -0.06. In the study period, average spring $T_{max}$ was 17.6 $degreeC$ and summer SPEI was 0.51.
...
Within the study period for the dendrometer band analysis (1999-2003), Spring temperatures (March + April $T_{max}$) and summer SPEI values (4-month value of August) were similar to the average prior to the study period (1970-1998). Prior to the study period, the average spring $T_{max}$ was 10.9 $degreeC$ and summer SPEI was 0.13. In the study period, average spring $T_{max}$ was 11.2 $degreeC$ and summer SPEI was 0.20.
The summer of 1999 was abnormally dry, with precipitation of only 52 mm mo^-1^ in June-August compared to average monthly precipitation of 100+ mm[@dorangeville_peak_2022].
...
*Timing of measurements at Harvard Forest were similar, with the median start date of April 23 and median end date of October 30.* <!--Changes depending on when we calculate median. I think we can calculate it before removal of 1998 since we technically incude 1998 in our raw data, just remove it during our data cleaning. If you don't like that, it cahges to: start: april 21, end: October 27-->
All measurements from 1998 were dropped because of a late start date (May 26).
*1999 was an anomalous year where initial measurements were taken on January 5, but not taken again until April 15.* <!--I think we dropped this.Correct? CD: No, left in. This could influence growth in 1999 if "a" was selected based on pre-hydration, winter DBH, but I have faith that Sean's process to find "a" can handle this. That being said, it is unfortunate that 1999 is the only year with early measurements and very weird timing. Justification for my faith: If "a" uses prehydration, winter dbh, then we would expect the "a" in 1999 to be proportionally smaller than in other years. This smaller "a" coupled with an unchanged "b" (last measurement was about the same in 1999 as other years) would result in greater total growth (b-a). But we actually see that 1999 total growth is the smallest, indicating to me that the hot summer is driving the weirdness. -->
...
We also removed tree-years with small or negligible total growth ($\Delta DBH$ $\le 0.05$ mm; SCBI = 26, Harvard Forest = 253), and tree-years where first intraannual measurement was later than the first spring survey (trees that were missed in the initial census; SCBI = 22, Harvard Forest = 8).
...
(1) $g_{max}$ was $\ge 2.5$ standard deviations away from the mean for each site-xylem architecture group combination (SCBI = 3, Harvard Forest = 11);  
(2) timing variables ($DOY_{ip}$, $DOY_{25}$, $DOY_{50}$, $DOY_{75}$) were $\ge 2.5$ standard deviations away from the means for their site, xylem architecture group, and year (SCBI = 74, Harvard Forest = 101).
...
<!--Hypothesis testing -->
Correlation between the dendrometer band-derived growth parameters ($DOY_{25}$, $DOY_{50}$, $DOY_{75}$, $L_{PGS}$, $g_{max}$, and $\Delta DBH$, Fig. `r schematic_summary`) and CTW $T_{max}$ (at weekly or monthly resolution, as described above) were assessed using a linear mixed model in a hierarchical Bayesian framework.
...
To rule out the possibility that observed patterns were strongly influenced by summer drought, we examined the relationship between spring temperatures and summer Standardized Precipitation Evapotranspiration Index (SPEI)[@vicente-serrano_multiscalar_2010].
