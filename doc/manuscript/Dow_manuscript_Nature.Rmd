...
**However, less is known about how spring temperatures affect tree stem growth[@huang_photoperiod_2020; @delpierre_chilling_2019; @rossi_critical_2008; @savage_coordination_2021], which sequesters carbon (C) in wood that has a long residence time in the ecosystem[@xue_global_2017;@russell_residence_2014].**
...
However, recent experimental and observational findings show that annual stem growth is not correlated with growing season length[@etzold_number_2022], annual productivity can be limited by sink factors[@keenan_timing_2015; @zani_increased_2020; @zohner_how_2021], and that positive effects of warm springs are compensated by negative effects of accumulation of seasonal water deficits[@buermann_widespread_2018]. These studies suggest that warmer spring temperatures may not have the expected positive effect on forest CO~2~ sequestration.

While responses of leaf phenology and seasonal _NEE_ to warming spring temperatures have been documented[@jeong_phenology_2011; @keenan_net_2014; @buermann_widespread_2018; @ibanez_forecasting_2010; @menzel_european_2006; @menzel_growing_1999;@finzi_carbon_2020], little is known about how the longest-lived component of fixed C in trees, the woody stem growth, is responding to warming spring temperatures[@huang_photoperiod_2020; @delpierre_chilling_2019; @rossi_critical_2008; @savage_coordination_2021]. 
This uncertainty has led to  a surge in recent studies documenting the climate sensitivity of stem growth timing and its link to annual growth[@dorangeville_peak_2021; @etzold_number_2022; @tumajer_limitation_2022; @zweifel_why_2021]; however, the role of spring temperatures in shaping the timing of growth remains unclear.
...
![**Figure 2 | Foliage (a,b) and stem growth (c,d) phenology at the Smithsonian Conservation Biology Institute (a,c) and Harvard Forest (b,d).** Panels (a-b) show ecosystem-level canopy foliage phenology from 2001-2018, obtained from the MODIS Global Vegetation Phenology product (MCD12Q2.006), where G = Greenup, M=Mid-greenup, P=peak, and S=Senescence (i.e., beginning of green-down). Canopy foliage variables were extracted from a single pixel at the center of each forest plot. Only data flagged as "good" or "best" quality were included in the calculation of these phenology variables. Panels (c-d) show the dates at which stem growth milestones were achieved, on average, for sampled populations of ring-porous and diffuse-porous trees at SCBI (2011-2020) and Harvard Forest (1998-2003). Mean temperature was calculated for each xylem architecture/site combination over the respective critical $T_{max}$ window, then turned into a ratio and assigned a color on a gradient where the coldest year in the sample is blue and the warmest is red.](tables_figures/DOYtiming_allyears.png){ width=25% }
...
However, inconsistent with the concept that an earlier start to growth would increase annual woody productivity, we demonstrate that warmer springs hasten the cessation of stem expansion and thereby have negligible effect on total annual growth for most species and locations (Fig. 3).
We demonstrate that warmer springs either have little effect on, or even hasten, the cessation of peak stem expansion and thereby have negligible effect on total annual stem growth for most species and locations (Fig. `r tree_rings`). 
Our observations suggest that the cessation of rapid stem expansion, which occurs mid-summer near the time of peak canopy greenness (`r EDF_growth_curves_all`)[@finzi_carbon_2020; @etzold_number_2022], is likely driven by cues other than photosynthate limitation, such as water stress, daylength, or sink limitation[@zweifel_why_2021; @etzold_number_2022;@zani_increased_2020;@keenan_timing_2015;@fu_variation_2014].
...
Cite delpierre 2016 around here. Delpierre et al. 2016 found that stem growth was not correlated with carbon inputs, agreeing with the results here. However, the authors 
...
We fit a five-parameter logistic growth model[@mcmahon_general_2015] to dendrometer band data from each tree-year to define stem growth milestones and growth rates (Fig. `r schematic_summary`). In particular, we model the observed diameter at breast height (DBH) on a given day of the year (DOY; *i.e.*, julian days) as: 
...
The model outputs two additional variables, $a$ and $b$, which represent the beginning and end DBH in each model year. We used the first and last manual dendrometer band measurements as starting points to calculate these two variables. With this method, initial measurements too late in the growing season which miss the start of the growing season can cause incorrect estimations of $a$ and $b$. The 1998 survey at Harvard Forest recorded first measurements on May 26, which is less than 2 weeks away from the average DOY25 at this site. This late start made the calculation of timing and total growth variables unreliable, so we removed tree years in 1998 from our analysis. 
...
(1) single day growth rates were $\ge 2$ standard deviations away from the mean for each xylem architecture (SCBI = 26, Harvard Forest = 24); 
(2) timing variables ($DOY_{ip}$, $DOY_{25}$, $DOY_{50}$, $DOY_{75}$) were $\ge 3$ standard deviations away from the means for their xylem architecture group, year, and site (SCBI = 71, Harvard Forest = 226); 
(3) tree-years with small or negligible total growth ($\Delta DBH$ $\le 0.05$ mm; SCBI = 8, Harvard Forest = 137); 
(4) tree years where first intraannual measurement was later than the first spring survey (trees that were missed in the initial census; SCBI = 20, HF = 6).
At both sites the tag years removed through this method were proportional to the original sample size, indicating that no species or size class was disproportionately removed compared to others.
This process was repeated using 2.5 and 3 standard deviations and the results of analyses were not qualitatively different. 

<!-- Leaf phenology -->

Canopy foliage phenology data for the years 2001-2018 were extracted for SCBI and Harvard Forest from the MCD12Q2 V6 Land Cover Dynamics product (a.k.a. MODIS Global Vegetation Phenology product)[@friedl_mcd12q2_2019] via Google Earth Engine. For each year at each site, we extracted data from the pixel containing the center of each forest plot (resolution of 500m). Using the daily MODIS 2-band Enhanced Vegetation Index data (EVI2) , the product yields the timing of phenometrics (vegetation phenology) over each year, including timing of greenup, midgreenup, and senescence as used in this study. Data points were kept if they were flagged as "good" or "best" quality.
