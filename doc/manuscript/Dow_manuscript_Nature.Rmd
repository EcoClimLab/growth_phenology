...
First, C that is fixed in late summer or fall may be used for woody growth that is not apparent in the current year's diameter increment: either used to provide extra thickening of cell walls, a process that lags behind stem expansion[REF], or saved within trees as non-structural carbohydrates and used towards growth the following year. Indeed, tree-ring evidence from SCBI indicates that favorable growing conditions during this time enhance the next year's growth[@helcoski_growing_2019].
Second, the additional C may be allocated, likely in the following growing season, to short-lived C pools such as reproduction, foliage, fine roots, or root exudates. Most of this carbon would have a relatively low residence time in the ecosystem, although a small fraction would be incorporated into soil organic carbon pools with residence times rivaling that of wood [REFS].
Notably, both of these involve lags of a year or more and would not be captured in interannual variation in NEE, implying that the long-term effect of lengthened growing season on NEE may not be captured in studies, including this one, that consider time frames of a year or less[@keenan_net_2014;@zhou_explaining_2016; @fu_maximum_2019].

*(KAT edits end here)*

*There is a difference between interannual variation and directional change: the latter is likely to result in acclimatization, and eventually changes in forest composition/ adaptation.* 
...
Warm spring temperatures result in earlier leaf-out (Fig. `r DOY_timing` , REFS), an earlier start to net C sequestration (REFS), and, as shown here, earlier initiation of stem growth (Figs. `r schematic_summary`- `r DOY_timing`).
