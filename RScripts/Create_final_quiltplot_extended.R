...
all_dcc_output_hf <- read_csv("Results/tree_cores/quiltplots/plot_data/HF/all.dcc.output_hf_EXTENDED.csv")#created in HF_quiltplot.R
all_dcc_output_hf$site <- paste0("HF_", all_dcc_output_hf$Species)
all_dcc_output_other <- read_csv("Results/tree_cores/quiltplots/plot_data/Other/other_core_corr_EXTENDED.csv")#created in Other_quiltplot.R
...
all_dcc_output_scbi <- read_csv("Results/tree_cores/quiltplots/plot_data/SCBI/scbi_core_corr_EXTENDED.csv")#Created in scbi_quiltplot.R
...
# write.csv(SI_table, file = paste0("doc/manuscript/tables_figures/", "chronology_table.csv"), row.names = FALSE)
  ...
  #
  write.csv(X, file = paste0("results/quiltplot_data/", v, "_quilt_plot_data_EXTENDED.csv"), row.names = FALSE)
}
...
tmx_data <- read.csv("results/quiltplot_data/tmx_quilt_plot_data_EXTENDED.csv")
...
tmn_data <- read.csv("results/quiltplot_data/tmn_quilt_plot_data_EXTENDED.csv")
...
#colnames(x) <- seq(1, ncol(x), 1)
    ...
    png(paste0("results/tree_cores/quiltplots/plot_images/", "monthly_", "correlation", "other","EXTENDED", v,WT, ".png"), res = 150, width = 169, height = 2*169, units = "mm", pointsize = 10)
