...
all_stems <- Stem[NULL ,c(1:15)]
all_stems <- rbind(all_stems, Stem$dbh2)
colnames(all_stems) <- paste0("X", Stem$DOY)

for(i in 1:length(all_stems_intra)){
  Stem <- all_stems_intra[[i]]
  Stem <- subset(Stem, year == 2019)
...
write.csv(Param.df, file = "SCBI_hi_lo_lg5_2019.csv", quote = FALSE, row.names = FALSE)
