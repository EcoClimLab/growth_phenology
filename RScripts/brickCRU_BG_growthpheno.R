...
ForestGEO_sites <- read_csv("~/GitHub/growth_phenology/Data/tree_rings/locations.csv")

# points of interest
ForestGEO_sites$lat<- as.numeric(ForestGEO_sites$Latitude)
ForestGEO_sites$lon<- as.numeric(ForestGEO_sites$Longitude)
...
gzfilenames <- c("cru_ts4.04.1901.2019.tmn.dat.nc.gz","cru_ts4.04.1901.2019.tmx.dat.nc.gz")

  list.files(path = '"~/GitHub/growth_phenology/climate data/CRU"', pattern = ".nc.gz$") # replace with your local path name
