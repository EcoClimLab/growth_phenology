...
#Problem: some temps are appearing as NaN (not a number). As a result, most data is taken from NCDC
...
data <- data[complete.cases(data$Air_avgtmp1),]
...
missingdates <- read_csv("C:/Users/world/Desktop/Github/growth_phenology/climate data/SCBI_mettower_dailymeans_sensor1.txt.missing_dates.csv", col_names = "date")
missingdates$date <- strptime(as.character(missingdates$date), "%m/%d/%Y")
missingdates$date <- format(missingdates$date, "%Y-%m-%d")
...
#dates in NCDC data
missingdates_data <- NCDC_NOAA_precip_temp[NCDC_NOAA_precip_temp$newdate %in% missingdates$date,]
#No dates in NCDC data
missingdates_nodata <- missingdates[!(missingdates$date %in% missingdates_data$newdate),]
...
missingdates_nodata$newdate <- as.Date(missingdates_nodata$date)
...
SCBI_mettower_data_sensor1 <- read_csv("C:/Users/world/Desktop/Github/growth_phenology/climate data/SCBI_mettower_data_sensor1.csv")